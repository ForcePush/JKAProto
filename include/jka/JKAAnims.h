...
namespace JKA {
    typedef struct animation_s {
        uint16_t    firstFrame;
        uint16_t    numFrames;
        int16_t     frameLerp;  // msec between frames
    ...
    const std::array<animation_t, MAX_TOTALANIMATIONS> & getHumanoidAnimationsInfo();

    constexpr auto SABER_ANIM_GROUP_SIZE = (BOTH_A2_T__B_ - BOTH_A1_T__B_);
}
